...
type MuxAdapter struct {
	...
	mux         *http.ServeMux
	routes      map[string]map[string]common.HandlerFunc // method -> path -> handler
...
func NewMuxAdapter(config ...*AdapterConfig) *MuxAdapter {
	...
	return &MuxAdapter{
		...
		routes:      make(map[string]map[string]common.HandlerFunc),
...
func (a *MuxAdapter) RegisterRoute(route common.RouteDefinition) error {
	a.mu.Lock()
	defer a.mu.Unlock()
	...
	if _, exists := a.routes[method]; !exists {
		a.routes[method] = make(map[string]common.HandlerFunc)
	}
...
func (a *MuxAdapter) Use(middleware common.MiddlewareFunc) {
	a.mu.Lock()
	defer a.mu.Unlock()
