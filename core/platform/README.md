...
func GetUser(ctx *common.Context) error {
...
func ToGinHandler(handler common.HandlerFunc) gin.HandlerFunc {
    return func(c *gin.Context) {
        ctx := common.NewContext(c.Writer, c.Request) // ✅ Direct access!
        handler(ctx)
    }
}
...
func WrapHandler(handler common.HandlerFunc) any {
...
func HelloHandler(ctx *common.Context) error {
...
func ProtectedHandler(ctx *common.Context) error {
    if !authorized {
...
```go
func CreateUser(ctx *common.Context) error {
...
```go
gonestMiddleware := func(next common.HandlerFunc) common.HandlerFunc {
...
1. **Write handlers platform-agnostic** - Use `common.Context` only
