...
var UserController = gonest.NewController(func (controller *gonest.Controller) {
  ...
  controller.Route(gonest.HttpGet, "/:user_id", func (route *gonest.Route) {
    ...
    route.Handler(func(ctx *gonest.Context) {
      // MustParams e MustHeaders usam o MESMO Schema (passado explicitamente,
      ...
      headers := gonest.MustHeaders[*CustomHeaders](ctx, customHeadersSchema)
  ...
  controller.Route(gonest.HttpGet, "/", func (route *gonest.Route) {
    ...
    route.Handler(func(ctx *gonest.Context) {
      // MustQuery[T](ctx, schema) usa o MESMO Schema para Validação Runtime
      query := gonest.MustQuery[*ListUsersQuery](ctx, listUsersQuerySchema)
...
# Must* recebe o Schema explícito (decisão tomada, AD-019)

A hipótese original cogitava prender o parsing DIRETO no valor `*Schema`
(`ExParams.MustParseParams(ctx)`, estilo método) -- descartada: exigiria
`Schema` virar genérico (`Schema[T]`), e `internal/schema` é hoje pacote
FOLHA (zero import próprio) enquanto `route`/`validate`/`openapi` TODOS o
importam; um método que precisasse de `*execution.Context`/`*route.Route`
introduziria ciclo de import na volta. O objetivo real (obrigar o Schema a
...
`MustParams`/`MustQuery`/`MustJsonBody` passam a receber o `*Schema` como
2º argumento em vez de fazer lookup por tipo num registry global -- não dá
mais pra chamar `gonest.MustParams[*ParamsDTO](ctx)` sem ter, em mãos, o
...
- Converte um footgun de RUNTIME (schema esquecido só panica na 1ª request
  real que bater na rota) num erro de COMPILAÇÃO (Schema literalmente não
  existe no escopo da chamada).
...
- Não se aplica a `MustInject`/`MustInjectAll` -- ver AD-019 em STATE.md
  pro raciocínio completo (DI existe pra DESACOPLAR consumer de provider,
  método preso no valor do Provider reintroduziria esse acoplamento; e
  `MustInjectAll` já prova que não existe "o valor único" no caso
  multi-bind).
...
var controller = gonest.NewController(func(controller *gonest.Controller) {
  controller.Route(gonest.HttpGet, "/:user_id", func(r *gonest.Route) {
    ...
    r.Handler(func(ctx *gonest.Context) {
      ...
      query := gonest.MustQuery[*exQuery](ctx, exQuerySchema)
      params := gonest.MustParams[*exParams](ctx, exParamsSchema)
      body := gonest.MustJsonBody[*exBody](ctx, exBodySchema)
      // ...
    })
  })
})
