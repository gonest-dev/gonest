...
// Context is the single parameter passed to every Query/Mutation/
...
func NewGraphqlContext(args execution.Parseable, done <-chan struct{}) *Context {
	return &Context{args: args, done: done}
}
...
func (c *Context) Args() execution.Parseable {
	return c.args
}
