...
func SetupSwagger(a *app.App, uiPath string, doc *OpenAPI, options SwaggerOptions) error {
	...
	if err := adapter.RegisterRoute(route.HttpGet, options.JsonDocumentUrl, func(c *execution.HttpContext) {
		c.Response().Json(doc.Document()) //nolint:errcheck // best-effort write, mirrors other Handler bodies in this codebase
	}); err != nil {
		return err
	}

	html := renderSwaggerUIHTML(options)
	if err := adapter.RegisterRoute(route.HttpGet, uiPath, func(c *execution.HttpContext) {
		c.Response().Html(html) //nolint:errcheck // best-effort write, mirrors other Handler bodies in this codebase
	}); err != nil {
		return err
	}
