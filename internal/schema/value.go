...
// mudança em código já testado"). The returned *PropertyBuilder is s's
...
func NewValue(valueType reflect.Type) (*Schema, *PropertyBuilder) {
	...
	Register(valueType, s)
	return s, pb
}

// IsValue reports whether s was constructed via NewValue (a standalone
...
func (s *Schema) IsValue() bool {
	return s.isValue
}

// ValueProperty returns the single implicit *PropertyBuilder registered by
// NewValue. Panics if s was not built via NewValue -- same defensive shape
