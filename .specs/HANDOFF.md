...
- **Milestone 3 em progresso:**
  ...
  - **AD-009 registrada (STATE.md):** pedido do usuário — pacote raiz consolidado de 13 arquivos (`app.go`/`controller.go`/`exception.go`/`guard.go`/`inject.go`/`interceptor.go`/`middleware.go`/`module.go`/`options.go`/`param.go`/`pipe.go`/`provider.go`/`scope.go` + testes) pra um único `gonest.go`/`gonest_test.go`, commit `be09f7e`. Gate verde, gofmt limpo, sem flakiness. Salvo como memória de feedback pra persistir daqui pra frente — próximos re-exports (Filter, etc) vão pro `gonest.go` existente, não arquivo novo.
  - **AD-010 registrada (STATE.md):** pedido do usuário — `internal/httpctx`→`internal/execution` (tipo continua `Context`), `internal/fiberapp`→`internal/adapter/fiber` (pacote `fiber`, arquivo `fiber.go`, prepara `internal/adapter/` pra multi-adapter futuro). Commit `f2bdff3`. Rename puro, gate verde, `go vet`/`gofmt` limpos. Docs de features já completas ficam como registro histórico (não atualizadas); `TESTING.md` atualizado.
