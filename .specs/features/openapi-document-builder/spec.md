...
Milestone 7's first, self-contained piece: `gonest.NewOpenAPI(version, fn)` builds a document-LEVEL metadata object (title, description, version, contact, license, bearer auth) with zero dependency on `internal/metadata`/`internal/route` -- it's a standalone builder, same shape INSIGHT.md's own bootstrap example (`# exemplo de bootstrap completo`) already fully specifies. `SetupSwagger` (serving it) and actual schema generation from routes/metadata are SEPARATE features (this milestone's other 2 ROADMAP items) -- this feature is ONLY the document builder itself.

## Goals

- [ ] `gonest.NewOpenAPI(version string, fn func(b *OpenAPI)) *OpenAPI` -- INSIGHT.md's own call shape (`gonest.NewOpenAPI("3.1.0", func (b *gonest.OpenAPI) {...})`)
- [ ] `OpenAPI.Title(s string)`, `Description(s string)`, `Version(s string)` -- basic metadata setters
- [ ] `OpenAPI.Contact(name, url, email string)` -- INSIGHT.md's exact call shape (3 positional strings, not a struct)
- [ ] `OpenAPI.License(name, url string)` -- INSIGHT.md's exact call shape (2 positional strings)
- [ ] `OpenAPI.BearerAuth()` -- zero-arg, marks the document as using bearer token auth (OpenAPI's `securitySchemes`)
...
| Actual OpenAPI 3.1 JSON/YAML SERIALIZATION of the document                                                          | This feature only builds the in-memory `*OpenAPI` -- turning it into the wire format is "Schema Generation from Metadata"'s concern (it needs paths/schemas to produce a useful document anyway, no reason to serialize an empty shell here) |
...
**User Story**: As a gonest user, I want `gonest.NewOpenAPI("3.1.0", func (b *gonest.OpenAPI) { b.Title(...); b.Description(...); b.Version(...); b.Contact(...); b.License(...); b.BearerAuth() })` (INSIGHT.md's own bootstrap example) to compile and store every field correctly.

**Acceptance Criteria**:

1. WHEN `NewOpenAPI(version, fn)` is called THEN system SHALL construct a `*OpenAPI`, store `version` (the OpenAPI SPEC version, e.g. `"3.1.0"` -- distinct from `Version(s)`, which sets the API's OWN version per INSIGHT.md's `b.Version(config.OpenApi.Version)`), run `fn(doc)`, and return `doc`
...
**Independent Test**: reproduce INSIGHT.md's bootstrap example verbatim (all 6 builder calls), assert every getter returns exactly what was set, plus the OpenAPI spec version string passed to `NewOpenAPI` itself.
...
| -------------- | ----------------------------------------------- | ----- | ------ |
| OD-01          | P1: NewOpenAPI constructs, runs fn, returns doc | Done  | Done   |
...
- [x] INSIGHT.md's bootstrap example (`NewOpenAPI` block) compiles and stores every field correctly
