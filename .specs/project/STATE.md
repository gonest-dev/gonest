...
**Current Work:** Milestone 8 ("Test App Bootstrap") **COMPLETE** -- executado de ponta a ponta nesta sessão (T0 bootstrap de 3 fases + AD-008 reversão, T1 interface MustInject, T2 MustInjectAll, T3 MustNewTestApp/TestBuilder/MustOverride, T4 re-exports+verificação), 5 commits (`450656a`,`94ceb9f`,`81f781e` + 2 docs). Suite inteira (`go test ./... -race`) verde, ZERO mudança de asserção em teste pré-existente (só timing-dependent tests atualizados pro novo contrato deferido, mesmo padrão idempotente que Provider/Controller/Pipe já usavam). 4 SPEC_DEVIATIONs documentados em `.specs/features/test-app-bootstrap/tasks.md` (owner de MustInject/MustInjectAll widened `module.Owner`→`any`; `module.MiddlewareRef`/`FilterRef` novo pra quebrar ciclo de import `module`↔`middleware`/`filter`; `TestApp` mora em `internal/app/testapp.go`, não pacote próprio; override copy-in-place guardado por `real.Type()==placeholder.Type()`). "HTTP Test Client" (Milestone 8's 2ª feature) e Emitter/Scheduler/Terminus (Milestones 9-11) continuam SÓ especificadas, agora DESBLOQUEADAS (dependiam do bootstrap de 3 fases, que já existe). PRÓXIMA SESSÃO: escolher qual das 4 pendentes (HTTP Test Client, Emitter, Scheduler, Terminus) executar a seguir -- todas usam o mesmo mecanismo `New*`+`MustInject`/`MustInjectAll` que este trabalho acabou de habilitar. Reler cada feature's design.md antes de despachar T0 dela (mesmo aviso de sempre: assinaturas são melhor-julgamento do orquestrador de quando foram escritas).
