...
**Current Work:** Milestones 8 E 9 **COMPLETE** -- "Test App Bootstrap", "HTTP Test Client" (Milestone 8) E "Emitter & Listener" (Milestone 9) executadas de ponta a ponta nesta sessão, 7 commits de código (`450656a`,`94ceb9f`,`81f781e`,`96f3d57`,`1e08298` + 3 docs). Suite inteira (`go test ./... -race`) verde, ZERO mudança de asserção em teste pré-existente ao longo de TODA a sessão. Emitter introduziu mecanismo novo (não previsto em nenhum design.md, decisão tomada durante execução): `internal/inject.RegisterGlobalSingleton`/`GlobalSingletonFor`, um registro genérico de "framework singleton resolvível de qualquer módulo sem registro", checado por `MustInject[T]` ANTES do dispatch `directResolver`/placeholder -- reutilizável por Scheduler/Terminus se precisarem do mesmo padrão. SPEC_DEVIATIONs de "Test App Bootstrap" seguem documentados em `.specs/features/test-app-bootstrap/tasks.md`. Scheduler/Terminus (Milestones 10-11) continuam SÓ especificadas, DESBLOQUEADAS. PRÓXIMA SESSÃO: escolher entre Scheduler ou Terminus -- Scheduler provavelmente TAMBÉM precisa de decisão de arquitetura nova (execução isolada por job, recover próprio, provavelmente roda em goroutine própria tipo cron -- nenhuma lib de cron ainda no go.mod, verificar se precisa adicionar dependência externa). Reler cada feature's spec.md antes de despachar (nenhuma tem design.md ainda -- foram especificadas só até spec.md nesta sessão anterior).
