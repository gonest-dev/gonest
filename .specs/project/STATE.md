...
**Current Work:** Milestones 1, 2, 3, 4 e 5 **COMPLETE**. Milestone 5 (Metadata Builder — Array & Object) fechado: "Array Builder" (T1-T2) e "Object Builder" (T1-T2) DONE. `ArrayMetadata` (`internal/metadata/array.go`, commits `31a76b3`/`1c5ea45`) — builder DUAL-STATE (campo container + item sintético). `ObjectMetadata` (`internal/metadata/object.go`, commits `6a602fb`/`1e734ba`) — builder SINGLE-STATE (sem item, campo INTEIRO é o objeto), mais simples que Array por design (ver design.md's Tech Decisions). Ambos usam `Items(fn)`/`Object(fn)` CALLBACK-shaped (AD-011) -- pra Array isso resolve ambiguidade real de escopo (campo vs item); pra Object é só consistência de API (não tem ambiguidade nenhuma pra resolver, `Required`/`Description` dentro ou fora do callback são idênticos, mesmo `*PropertyBuilder`). Próxima: especificar Milestone 6 (Runtime Validation — `MustJsonBody[T]`/`MustParam[T]` validando contra a metadata já registrada, ver ROADMAP.md).
