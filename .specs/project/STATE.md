...
**Current Work:** Milestones 1, 2, 3 e 4 **COMPLETE**. Milestone 5 (Metadata Builder — Array & Object) em progresso: feature "Array Builder" (T1-T2) DONE. `ArrayMetadata` (`internal/metadata/array.go`, commits `31a76b3`/`1c5ea45`) — builder DUAL-STATE, primeiro tipo desta feature que precisa de 2 `*PropertyBuilder` simultâneos (campo container + item sintético, nunca registrado em `Metadata.properties`). `Items(fn func(m *ArrayMetadata))` é CALLBACK (não builder linear puro) -- usuário revisou INSIGHT.md nesta sessão pra resolver ambiguidade "Required()/Description() depois de Items().String() aplica no campo ou no item?" fazendo o callback receber o MESMO `*ArrayMetadata` sempre, então `m.Required()` é sempre método do campo, `m.String()`/`m.Integer()`/etc são sempre método que configura o item (delega pro `item *PropertyBuilder` sintético, reusando `StringMetadata`/`NumericMetadata` já existentes de graça). Isso PARCIALMENTE reverte AD-002 (só pra Array/Object -- resto dos branches primitivos continua builder linear puro). Próxima: especificar "Object Builder" (2ª e última feature de Milestone 5, ver ROADMAP.md) -- mesmo padrão dual-state provavelmente reaplica.
