...
**Milestone 11 (Terminus) REESPECIFICADO** (2026-07-15, sem execução ainda): usuário reescreveu INSIGHT.md's exemplo de Terminus/health, removendo o design antigo (`NewHealthCheck`/`Module.HealthChecks`/`App.UseHealthCheck`) -- agora health check é só um `Controller` comum usando `MustInjectAll[Connectable]` (já existe desde Milestone 8/9), sem NENHUM tipo novo de bootstrap. Escopo caiu pra praticamente zero código novo de framework -- só 2 primitivos de uso geral faltando: `Context.SendString` (`internal/execution`, novo `Responder.SendString`) e `gonest.HttpStatusOk`/`HttpStatusServiceUnavailable` (constantes raiz, INSIGHT.md já assumia que existiam em MÚLTIPLOS exemplos, não só Terminus). Ver `.specs/features/terminus-health/spec.md` revisado. SEM dependência bloqueante agora -- pode ser executado a qualquer momento, antes ou depois de Scheduler.

Scheduler (Milestone 10) continua SÓ especificado (spec.md de 2026-07-14), DESBLOQUEADO pelo bootstrap de 3 fases, mas ainda NÃO revisado nesta sessão -- provavelmente precisa de decisão de arquitetura própria (execução isolada por job/goroutine, nenhuma lib de cron no go.mod ainda). PRÓXIMA SESSÃO: Terminus é a aposta mais barata pra executar primeiro (escopo mínimo, sem dependência); Scheduler exige mais decisão de design antes de começar.
