...
**Current Work:** Milestones 1-6 **COMPLETE**. Milestone 7 (OpenAPI Generation) em progresso: "OpenAPI Document Builder" (commit `9b08afd`) DONE -- builder mecânico e autocontido (`NewOpenApiDocument`/`Title`/`Description`/`Version`/`Contact`/`License`/`BearerAuth`), zero dependência de `internal/metadata`/`internal/route`. Faltam 2 features de Milestone 7: "Schema Generation from Metadata" (como rotas linkam pra request/response schemas -- AINDA NÃO DISCUTIDO com usuário, sem exemplo em INSIGHT.md, mesma classe de ambiguidade que "Param/Query Validation" teve no início) e "Swagger UI Setup" (depende da primeira existir). Próxima: discutir escopo de "Schema Generation from Metadata" antes de especificar (mesma classe de risco de "Param/Query Validation" -- pode crescer bastante uma vez discutido, ver AD-013 abaixo pro precedente desta sessão). Milestone 6 (Runtime Validation) fechado: "JSON Body Validation" e "Param, Query & Custom Validation" (T0-T4 cada), ver AD-012/AD-013 abaixo.
