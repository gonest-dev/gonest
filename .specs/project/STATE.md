...
**Current Work:** Milestones 1, 2 e 3 **COMPLETE**. Milestone 4 (Metadata Builder — Primitivos) em progresso: "Metadata Registration Core" e "String-family Branches" DONE. Feature "String-family Branches" (T1-T2, commit `be6d062`) — `PropertyBuilder` estendido com campo `format` + 10 métodos de branch (`String`/`Email`/`Uuid`/`Uri`/`Hostname`/`Ipv4`/`Ipv6`/`Password`/`Byte`/`Binary`), `StringMetadata` novo (mesmo pacote `internal/metadata`) com `Min`/`Max`/`Pattern` + os 4 comuns REDECLARADOS manualmente (não confiar em promoção de embedding). Decisão-chave: `format` persiste no `PropertyBuilder` COMPARTILHADO (não no wrapper `StringMetadata` descartável) — resolve o problema de "branch escolhido precisa sobreviver mesmo se o wrapper específico for descartado". Esse padrão (embed+redeclare, format no objeto base) é o molde mecânico que "Numeric & Boolean Branches" e "Date/Time Branches" vão repetir. Próxima: especificar "Numeric & Boolean Branches" (3ª feature de Milestone 4, ver ROADMAP.md).
