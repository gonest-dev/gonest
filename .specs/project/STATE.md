...
**Current Work:** Milestones 1-7 **COMPLETE**. ROADMAP v1 (Milestones 1-8) TOTALMENTE ESPECIFICADO: "Test App Bootstrap" (spec+context+design+tasks) e "HTTP Test Client" (spec, bloqueada em "Test App Bootstrap") prontas. ALÉM disso, "Future Considerations" promovido: Emitter (Milestone 9), Scheduler (Milestone 10), Terminus/health (Milestone 11) ganharam spec.md cada, baseadas em exemplos JÁ existentes no INSIGHT.md (corrigido um bug real no exemplo de Emitter -- `provider.Constructor` com parâmetro de dependência direto, incompatível com o mecanismo real de Provider). NENHUMA dessas 4 features (Test App Bootstrap, HTTP Test Client, Emitter, Scheduler, Terminus) teve execução iniciada -- todas são SÓ especificação, a pedido explícito do usuário. Multi-adapter/CLI/microservices continuam em "Future Considerations" (sem exemplo concreto no INSIGHT.md, não especificados). PRÓXIMA SESSÃO: ler `.specs/features/test-app-bootstrap/{context,spec,design,tasks}.md` INTEIROS antes de despachar qualquer sub-agent de T0 -- é a dependência bloqueante de TODAS as outras 4 features pendentes (Emitter/Scheduler/Terminus/HTTP Test Client todas usam o mesmo mecanismo `New*`+`MustInject` que só funciona corretamente com o bootstrap de 3 fases). design.md tem aviso explícito: assinaturas são melhor-julgamento do orquestrador, não verificadas linha-a-linha contra o código atual.

---

## Recent Decisions (Last 60 days)

### AD-015: Bootstrap de 3 fases (Provider → Controller → Pipeline-stage) + reversão de AD-008, pra viabilizar DI por interface (2026-07-14, SÓ ESPECIFICADO, execução pendente)
