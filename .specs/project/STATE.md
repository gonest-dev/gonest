...
**Current Work:** Milestones 1-5 **COMPLETE**. Milestone 6 (Runtime Validation) em progresso: feature "JSON Body Validation" (T0-T4) DONE. `internal/validate.MustJsonBody[T]` (re-exportado na raiz, commits `d012c7e`→`a9bbda9`) — primeiro consumidor real da metadata (Milestones 4-5 só construíam, nunca liam). Descobriu e resolveu 2 gaps bloqueantes pré-existentes durante o design (ver AD-012): (1) `Min`/`Max`/`Pattern`/`item`/`ref`/etc viviam só nos wrappers descartáveis, nunca sobreviviam além da closure de declaração — relocados pro `PropertyBuilder` compartilhado (T0, zero mudança de API pública, 4 features já fechadas continuam passando sem tocar teste nenhum); (2) `String()`/`Boolean()` ambos tinham `format==""`, indistinguíveis — campo `kind` novo resolve. Registro global (T1) auto-popula via `NewMetadata[T]`, permite `MustJsonBody[T]` achar a metadata sem argumento explícito. Validador (T3) é recursivo de verdade (Array valida item+quantidade, Object recursa via `ref`) e COLETA todas violações numa request (decisão explícita do usuário, ver context.md da feature). Próxima: especificar "Param/Query Validation" (2ª e última feature de Milestone 6, `MustParam[T]` + `Pipe` + coerção via metadata, ver ROADMAP.md).
