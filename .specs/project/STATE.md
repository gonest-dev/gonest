...
Scheduler (Milestone 10) é a ÚNICA feature restante do roadmap v1. Continua SÓ especificado (spec.md de 2026-07-14), DESBLOQUEADO pelo bootstrap de 3 fases, mas ainda NÃO revisado nesta sessão -- provavelmente precisa de decisão de arquitetura própria (execução isolada por job/goroutine, nenhuma lib de cron no go.mod ainda -- verificar se `Cron(name, expr, fn)` precisa de parser de cron expression externo ou se dá pra implementar sem dependência nova). PRÓXIMA SESSÃO: reler `.specs/features/scheduler/spec.md` inteiro antes de despachar qualquer trabalho -- decisão de arquitetura (cron parsing, isolamento de execução por job) ainda não tomada.
